...
- what_happened: 첫 문장에 기사에서 발생한 핵심 사건과 정량 팩트를 압축한다. 이후 문장은 숫자, 기간, 비교 기준, 계약 규모, 실적, 가이던스가 시장에서 무엇을 확인시켜 주는지까지 연결한다.
- context: what_happened의 연장선에서 왜 이 사건이 발생했는지 설명한다. 과거 지표 대비 변화, 전방 산업 수요, 경쟁사/정책/매크로 변수를 연결하되 what_happened의 수치를 반복 나열하지 않는다.
...
- what_happened와 context를 이어 붙였을 때 하나의 완성된 요약문처럼 읽혀야 한다. 같은 제목이나 같은 수치를 불필요하게 반복하지 않는다.
