...
class AIEngineTest(unittest.TestCase):
    ...
    def test_payload_enforces_deep_market_analysis_rules(self) -> None:
        ...
        self.assertIn("하나의 완성된 업무 보고 문단", messages)
        self.assertIn("업무 보고서 톤", messages)
        self.assertIn("출처 설명을 쓰지 않는다", messages)
        self.assertIn("주가 방향 예측이 아니라", messages)
        self.assertIn("시장 영향 및 체크포인트", messages)
        self.assertIn("수혜/부담", messages)
